...
public class DemoHelper {
    ...
    /**
     * sync contacts subscribed listener
     */
    private List<DataSyncListener> syncSubscribedListeners;
...
//        options.setImPort(6717);
    ...
    protected void setGlobalListeners() {
        ...
        syncSubscribedListeners = new ArrayList<>();
    ...
    public class MyContactListener implements EMContactListener {
        ...
        public void onContactDeleted(String username) {
            ...
            easePreferenceManager.removeSubscribedUser(username);
...
//                break;
            ...
            public void onNoticeList(Map<String, String> usersMap) {
                ...
                if (usersMap.containsKey("jids")) {
                    ...
                    if (!TextUtils.isEmpty(jids)) {
                        String[] split = jids.replace("[", "").replace("]", "").split(",");
                        for (String jid : split) {
                            //用户
                        }
                    }
                }
            ...
            public void onContactStatusChanged(Map<String, String> userStatusMap) {
                ...
                if (userStatusMap.containsKey("jid")) {
                    ...
                    if (!TextUtils.isEmpty(jid) && userStatusMap.containsKey("status")) {
                        ...
                        easePreferenceManager.setSubscribedUserStatus(jid, status);
                        notifyContactsSubscribedListener(true);
                    }
                }
            }
                    ...
                    if (!TextUtils.isEmpty(jids)) {
                        ...
                        try {
                            ...
                            for (int i = 0; i < jsonArray.length(); i++) {
                                ...
                                easePreferenceManager.setSubscribedUserStatus(jid, s);
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
    ...
    public void addSubscribedContactListener(DataSyncListener listener) {
        if (listener == null) {
            return;
        }
        if (!syncSubscribedListeners.contains(listener)) {
            syncSubscribedListeners.add(listener);
        }
    }


    public void removeSubscribedContactListener(DataSyncListener listener) {
        if (listener == null) {
            return;
        }
        if (syncSubscribedListeners.contains(listener)) {
            syncSubscribedListeners.remove(listener);
        }
    }
    ...
    public void notifyContactsSubscribedListener(boolean success) {
        for (DataSyncListener listener : syncSubscribedListeners) {
            listener.onSyncComplete(success);
        }
    }
