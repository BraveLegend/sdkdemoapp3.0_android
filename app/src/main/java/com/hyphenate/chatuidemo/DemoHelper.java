...
public class DemoHelper {
    ...
    private EasePreferenceManager easePreferenceManager;
    ...
    /**
     * sync contacts noticed listener
     */
    private List<DataSyncListener> syncContactNoticeListeners;
...
//        options.setImPort(6717);
        options.setRestServer("60.205.245.232:8083");
        options.setIMServer("60.205.245.232");
        options.setImPort(8082);
...
//            setCallOptions();
            ...
            initDbDao();
            easePreferenceManager = EasePreferenceManager.getInstance();
        }
    }
...
//                    intent = new Intent(appContext, VoiceCallActivity.class);
    ...
    protected void setGlobalListeners() {
        ...
        syncContactNoticeListeners = new ArrayList<>();
...
//        appContext.registerReceiver(callReceiver, callFilter);
    ...
    public class MyContactListener implements EMContactListener {
        ...
        public void onContactAdded(String username) {
            ...
            EMClient.getInstance().chatManager().addUsersToNoticeList(userIds, new EMCallBack() {
                ...
                public void onSuccess() {
                    EMLog.d(TAG, "addUsersToNoticeList onSuccess:" + username);
                    EMClient.getInstance().chatManager().getUserStatusWithUserIds(userIds, null);
                }

                @Override
                public void onError(int code, String error) {
                    EMLog.d(TAG, "addUsersToNoticeList onError:" + username + ",code=" + code + ",error=" + error);
                }
        ...
        public void onContactDeleted(String username) {
            ...
            ids.add(username);
            EMClient.getInstance().chatManager().deleteUsersFromNoticeList(ids, new EMCallBack() {
                @Override
                public void onSuccess() {
                    EMLog.d(TAG, "deleteUsersFromNoticeList onSuccess:" + username);
                    easePreferenceManager.removeNoticeUser(username);
                    notifyContactsSyncListener(true);
                }

                @Override
                public void onError(int code, String error) {
                    EMLog.d(TAG, "deleteUsersFromNoticeList onError:" + username + ",code=" + code + ",error=" + error);
                }
    ...
    protected void registerMessageListener() {
        messageListener = new EMMessageListener() {
            ...
            public void onNoticeList(Map<String, String> usersMap) {
                //通知自己的用户列表
                EMLog.d(TAG, "用户列表：" + usersMap.toString());
                if (usersMap.containsKey("jids")) {
                    String jids = usersMap.get("jids");
                    if (!TextUtils.isEmpty(jids)) {
                        ArrayList<String> noticedList = new ArrayList<>();
                        //获取用户
                        try {
                            JSONArray jsonArray = new JSONArray(jids);
                            for (int i = 0; i < jsonArray.length(); i++) {
                                String item = (String) jsonArray.get(i);
                                noticedList.add(item);
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
            ...
            public void onContactStatusChanged(Map<String, String> userStatusMap) {
                //通知自己的用户上下线状态改变
                EMLog.d(TAG, "用户状态改变：" + userStatusMap.toString());
                if (userStatusMap.containsKey("jid")) {
                    String jid = userStatusMap.get("jid");
                    if (!TextUtils.isEmpty(jid) && userStatusMap.containsKey("status")) {
                        String status = userStatusMap.get("status");
                        easePreferenceManager.setNoticeUserStatus(jid, status);
                        notifyContactsNoticeListener(true);
                    }
                }
            }
            ...
            public void onQueryUserStatusList(Map<String, String> userStatusMap) {
                //已订阅用户中对应ID用户的上下线状态列表
                EMLog.d(TAG, "查询用户状态列表：" + userStatusMap.toString());
                if (userStatusMap.containsKey("jids")) {
                    String jids = userStatusMap.get("jids");
                    EMLog.d(TAG, "查询用户状态列表：jids=" + jids);
                    if (!TextUtils.isEmpty(jids)) {
                        String status = userStatusMap.get("status");
                        try {
                            JSONArray jsonArray = new JSONArray(jids);
                            JSONArray jsonArray2 = new JSONArray(status);
                            for (int i = 0; i < jsonArray.length(); i++) {
                                String jid = jsonArray.optString(i);
                                String s = jsonArray2.optString(i);
                                easePreferenceManager.setNoticeUserStatus(jid, s);
                            }
                        } catch (JSONException e) {
                            e.printStackTrace();
                        }
            ...
            public void onAllContactsStatusList(Map<String, String> userStatusMap) {
                //所有已订阅用户上下线状态列表
                EMLog.d(TAG, "获取用户状态列表：" + userStatusMap.toString());
                if (userStatusMap.containsKey("jids")) {
                    String jids = userStatusMap.get("jids");
                    if (!TextUtils.isEmpty(jids)) {
                        String status = userStatusMap.get("status");
...
//        endCall();
    ...
    public void addContactNoticeListener(DataSyncListener listener) {
        if (listener == null) {
            return;
        }
        if (!syncContactNoticeListeners.contains(listener)) {
            syncContactNoticeListeners.add(listener);
        }
    }


    public void removeContactNoticeListener(DataSyncListener listener) {
        if (listener == null) {
            return;
        }
        if (syncContactNoticeListeners.contains(listener)) {
            syncContactNoticeListeners.remove(listener);
        }
    }
    ...
    public void notifyContactsNoticeListener(boolean success) {
        for (DataSyncListener listener : syncContactNoticeListeners) {
            listener.onSyncComplete(success);
        }
    }
